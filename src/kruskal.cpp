...
void Kruskal::recorrer(){
    ...
    while( aux_1 < cantidad_vertices){
        ...
        if(unir(aux_3, aux_5) > 0){
            aux_1++;
            cout << "Pasar de la lectura " << CYAN << vertices[aux_2-1].obtener_vertice()->obtener_titulo() << BLANCO << " a la lectura " <<  MAGENTA << vertices[aux_4-1].obtener_vertice()->obtener_titulo() << BLANCO << ", lleva un tiempo de: "<< VERDE << minimo << BLANCO << " minutos" << endl;
            costo_minimo += minimo;
        }
         matriz_pesos[aux_2-1][aux_4-1] = matriz_pesos[aux_4-1][aux_2-1] = INFINITO;
    }
    cout << " " << endl;
    cout << "⏳ Tiempo mínimo para leer todas las lecturas: " << costo_minimo << endl;
}
